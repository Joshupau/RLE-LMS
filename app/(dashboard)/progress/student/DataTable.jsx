...
const DataTable = ({ data }) => {
  ...
  const paginatedFilteredData = data
    ...
    const handleDeleteCase = async (id, status) => {
      try {
        if(status === true){
          toast({
            title: "Uh oh...",
            description: "Can't delete case once it is approved.",
            status: "Failed",
          });
          return
        }

          const response = await fetch(`/api/progress/case/${id}/delete`, {
            method: "DELETE",
              ...
              <TableRow key={Case.id}>
                ...
                <Dialog className="min-w-max">
                    ...
                    <DropdownMenuContent className="w-56">
                        ...
                        <DropdownMenuItem onClick={() => handleDeleteCase(Case.id, Case.status)}>
